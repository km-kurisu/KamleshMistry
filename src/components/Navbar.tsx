...
export default function Navbar() {
    ...
    return (
        <nav
            ref={navRef}
            className="sticky top-0 left-0 right-0 z-50 px-6 py-4 md:px-12 bg-transparent backdrop-blur-md transition-all duration-300 -translate-y-full opacity-0"
        >
            <div className="max-w-screen-2xl mx-auto flex items-center justify-between">
                ...
                <Link href="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                    ...
                    <span className="text-2xl md:text-3xl font-bold tracking-tighter text-foreground select-none font-highlightbold">
                        Kamlesh Mistry
                    </span>
                </Link>
                ...
                {isOpen && (
                    <div className="absolute top-full left-0 right-0 bg-background/95 backdrop-blur-xl border-b border-white/10 p-6 md:hidden flex flex-col items-center gap-6 shadow-2xl">
                        <button
                            onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")}
                            className="flex items-center gap-2 text-foreground/70 hover:text-foreground transition-colors p-3 rounded-full bg-foreground/5 w-full justify-center"
                            aria-label="Toggle Theme"
                        >
                        ...
                        {links.map((link) => (
                            <Link
                                key={link.name}
                                href={link.href}
                                className="text-lg uppercase tracking-widest text-foreground/80 hover:text-foreground font-sans w-full text-center py-2"
                        ...
                        <a
                            href="/KamleshMistry.pdf"
                            target="_blank"
                            className="px-8 py-3 border border-zinc-200 dark:border-white/20 text-foreground rounded-full font-medium hover:bg-foreground hover:text-background transition-all w-full text-center"
                        >
                            Resume
                        </a>
                    </div>
